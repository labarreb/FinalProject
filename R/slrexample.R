slr <- function(){
  ...
  if(f=="1") summ <- summary(e)
  j <- menu(c("Y","N"),title="Would you like a plot?")
  if(j=="1") {
    jplot <- function(){
      plot(get(cc)~get(dd),data=bb,col="green",pch=19,
           xlab=dd,ylab=cc,main="Linear Regression Plot")
      abline(e)
    }
  }
  return(list(if(exists("summ")){summ},if(exists("jplot")){jplot()}))
}
...
mlr <- function(){
  ...
  if(h=="1") summ <- summary(e)
  j <- menu(c("Y","N"),title="Would you like a plot(s)?")
  if(j=="1" && cb!=1) {jplot <- function(){
    par(mfrow=c(2,2))
    plot(e)}}
  else if(j=="1" && cb==1) {kplot <- function(){
    plot(as.formula(ddd),data=bb,col="green",pch=19,
         xlab=ccc[1],ylab=cc,main="Linear Regression Plot")
    abline(e)
  }}
  res <- list(if(exists("summ")){summ},if(exists("jplot")){jplot()},if(exists("kplot")){kplot()})
